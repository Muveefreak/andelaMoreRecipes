...
!function () {
  var a = {},
    ...
    return a.add("tabfocus", function (a) {
      function g(a) {
        ...
        function g(e) {
          ...
          }if (j = i.select(":input:enabled,*[tabindex]:not(iframe)"), c.each(j, function (b, c) {
            if (b.id == a.id) return h = c, !1;
          }), e > 0) {
            for (l = h + 1; l < j.length; l++) {
              if (g(j[l])) return j[l];
            }
          } else for (l = h - 1; l >= 0; l--) {
            if (g(j[l])) return j[l];
          }return null;
        }var h, j, k, l;if (!(9 !== b.keyCode || b.ctrlKey || b.altKey || b.metaKey || b.isDefaultPrevented()) && (k = c.explode(a.getParam("tab_focus", a.getParam("tabfocus_elements", ":prev,:next"))), 1 == k.length && (k[1] = k[0], k[0] = ":prev"), j = b.shiftKey ? ":prev" == k[0] ? g(-1) : i.get(k[0]) : ":next" == k[1] ? g(1) : i.get(k[1]))) {
          var m = d.get(j.id || j.name);j.id && m ? m.focus() : e.setTimeout(function () {
            f.webkit || window.focus(), j.focus();
          }, 10), b.preventDefault();
        }
      }var i = b.DOM;a.on("init", function () {
        a.inline && i.setAttrib(a.getBody(), "tabIndex", null), a.on("keyup", g), f.gecko ? a.on("keypress keydown", h) : a.on("keydown", h);
      });
    }), function () {};
  }), d("0")();
}();